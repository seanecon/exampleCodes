...
y <- hist(anaDf.raw.1[,'n.lag1.lap'], plot=FALSE, n=20)
plot(y, col=ifelse(y$mid<14,'black','white'), xlab='Observed number of MIRPs performed during the past 3 months')
box()
dev.off()
...
solveByQuarter = function(quarterLap, n.random.seeds, pickBestDoctors, picked.n.md){
  ...
  for (i in 1:nrow(quarterLap)){
    ...
    tmp=solveOptAllocation(quarterLap[i,'n.lap'],  right.G.list, seq(n.random.seeds),  pickBestDoctors, picked.n.md=picked.n.md)  
...
opt.by.st.quarter.best.doc=solveByQuarter(quarterLap, 50, T, 15)
#reduction
(sum(opt.by.st.quarter.best.doc$los.obs)-sum(opt.by.st.quarter.best.doc$los.opt))/sum(opt.by.st.quarter.best.doc$los.obs)
#0.40800789
opt.by.st.quarter.random.doc=solveByQuarter(quarterLap, 50, F, 15)

(sum(opt.by.st.quarter.random.doc$los.obs)-sum(opt.by.st.quarter.random.doc$los.opt))/sum(opt.by.st.quarter.random.doc$los.obs)
# 0.33711916
...
save(anaDf.raw.1, fitList.logLike, fitList, meanLosbyvolTile
     , tileList, lagvolvec, bsDegree, avgLearnCurveByYear, plotAvgLearnDf, cex.pch
     ...
     , lower.volpermd
     , upper.volpermd
     , file="Z:/j_scrdata/lapLearn/bsplineSensitivity_dec212013.RData")
#  
# 
