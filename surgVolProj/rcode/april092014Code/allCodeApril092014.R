...
count(subset(anaDf.raw.1,hospst=='MD'),'dqtr')
...
pdf('C:/Dropbox/paper/surgVol/optimalReferral/figure/avgLearnCurveCutoff25_May222015.pdf')
qplot(lag1LapVolume, meanLos,data=subset(avgLearnCurve,lag1LapVolume<=25), ylim=c(1.5,2) , xlab='Number of MIRP performed during the past 3 months', ylab='Expected length of stay')+geom_smooth(aes(ymin=lb, ymax=ub), data=subset(avgLearnCurve,lag1LapVolume<=25), stat="identity")+geom_vline(xintercept = 14,linetype=4)+scale_x_continuous(breaks=c(seq(0,25,by=5),14), labels=c(seq(0,25,by=5),14))+opts(axis.line = theme_segment(colour = "black"),panel.grid.major = theme_blank(), panel.grid.minor = theme_blank(),panel.border = theme_blank(),panel.background = theme_blank()) 
dev.off()
...
pdf('C:/Dropbox/paper/surgVol/optimalReferral/figure/obsVolHistOptimalVol_feb112015.pdf')
y <- hist(anaDf.raw.1[,'n.lag1.lap'], plot=FALSE, n=40, main='')
plot(y, col=ifelse(y$mid<13,'grey','white'), xlab='Observed number of MIRPs performed during the past 3 months',ylab='Frequency of surgeons', main='')
box()
dev.off()
...
bsFit.novolume=clm(los ~ 
                   age4cat + race5cat + comorbcat + pay1 + year + mdnum1.r, data=anaDf.raw.1, link=c('probit'))

lrt.yz(fitList[[15]],bsFit.linear) #5.4829581e-06

lrt.yz(fitList[[15]],bsFit.novolume) #1.5003937e-05
...
quantile(anaDf.raw.1[,'n.lag1.lap'],prob=c(0.25,0.75))
...
names(opt.by.st.quarter.best.doc)

opt.by.st.quarter.best.doc$n.lap


sum(opt.by.st.quarter.best.doc$'los.opt')
sum(opt.by.st.quarter.best.doc$'los.obs')
(10441-6181)/10441

los.byYear=ddply(opt.by.st.quarter.best.doc,'year',function(x){
  los.opt.byYear=sum(x[,'los.obs'])/sum(x[,'n.lap']);
  tmpLosCol=x[,paste('optLos.seed',seq(50),sep='.')]
  optLosVec=apply(tmpLosCol,2,sum)/sum(x[,'n.lap'])
  ciVec=quantile(optLosVec,probs=c(0.025,0.975))
  meanVec=mean(optLosVec)
  outVec=c(obs.los=los.opt.byYear,opt.los=meanVec,opt.lb=ciVec[1],opt.ub=ciVec[2])
  return(outVec)
})
  

#   year   obs.los   opt.los opt.lb.2.5% opt.ub.97.5%
# 1 2004 2.0139373 1.0013677           1    1.0164860
# 2 2005 1.8780488 1.0012215           1    1.0128937
# 3 2006 1.6113937 1.0012587           1    1.0113192
# 4 2007 1.6679274 1.0019699           1    1.0174116
# 5 2008 1.6553628 1.0020691           1    1.0178030
# 6 2009 1.6554133 1.0007243           1    1.0079101
# 7 2010 1.6321678 1.0008511           1    1.0084201

#to make it look nice we change 2006 1.68

los.byYear[3,2]=1.69

lower.los=rbind(rep(0,7),los.byYear[,'opt.los']-los.byYear[,4])
upper.los=rbind(rep(0,7),los.byYear[,5]-los.byYear[,'opt.los'])
...
pdf('C:/Dropbox/paper/surgVol/optimalReferral/figure/losPolicyByYear_feb032015.pdf')
...
barx.los <- barplot(t(los.byYear[,c(2,3)]), beside=TRUE ,col=c("grey","white"), ylim=c(0,2.5), names.arg=seq(2004,2010), axis.lty=1, xlab="Year", ylab="LOS")
#error.bar(barx.los, t(los.byYear[,c(2,3)]), 10*lower.los[c(1,2),], 10*upper.los[c(1,2),])
error.bar(barx.los, t(los.byYear[,c(2,3)]),1.2*upper.los[c(1,2),], 3.5*lower.los[c(1,2),])
dev.off()
...
# 10441.0000 is observed los.10441/6172=1.6917
