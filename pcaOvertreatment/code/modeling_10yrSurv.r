...
finalSample[,'race'] <- replaceValJoin.yz(finalSample[,'race'] #this is typically a data column
                                          ,list(c('1'),c('2'),c('3'),c('4'),c('5','6'))
                                          ,c('1','2','3','4','5'))[,2]

count(finalSample,'race')

compareVars.yz(snc.cleaned, finalSample)
...
predProb <- predict(allSampleFit,finalSample,type='prob') #predProb[,2] is 10 year death prob
finalSample1 <- grpnv.yz(data.frame(finalSample,surv10YrProb=predProb[,1]),'surv10YrProb',3,'surv10YrProb3Cat')
...
#we need to generate 9 plots., the denomintator should be all patients meet 12-bf 12 after and not die within 6months
predicted value of utilization by year.

finalSample1[,'riskStrata'] <- catvars2onecatvar.yz(finalSample1, c('.damicorisk','surv10YrProb3Cat'), ' | ')
...
ddply(finalSample1,'surv10YrProb4Cat',function(x){mean(x[,'surv10YrProb'])})
ddply(finalSample1,'surv10YrProb3Cat',function(x){mean(x[,'surv10YrProb'])})
# surv10YrProb4Cat         V1
