...
gen.mlogitFit <- function(subData, adjustedVars,depVar, refLevel){
 
  tmpAna <- dummyMat.yz(subset(subData,select=adjustedVars),adjustedVars)
  tmpAnaDf <- data.frame(subData[,depVar],tmpAna)
  names(tmpAnaDf) <- c(depVar,colnames(tmpAna))
  rm(tmpAna)
  anaDf <- mlogit.data(tmpAnaDf, shape="wide", choice=depVar)
  #head(anaDf)
  #names(tmpAnaDf)
  fitObj <- mlogit(formula(paste(depVar,paste('0',
                                                 PlusSignInbetween(setdiff(names(anaDf),c(depVar,'alt','chid')), factor_wrapper=0)
...
genPredictProb.ovtrt <- function(fitObj){
  ...
  for (i in 1:length(studyYears) ){
    ...
    XB.Jminus1 <- X.choiceInvariant %*% matrix(coef(fitObj),ncol=nchoice-1,byrow=TRUE)
...
# riskGrps <- c('hig | surv10Yr.low','mid | surv10Yr.low','low | surv10Yr.low')
# adjustedVars= c('raceCat','.damicorisk','sesCat','ageCat','chrlsonRecat','grdCat','indexyear','registry')
# saveObjPathObjName <- 'Z:/j_scrdata/pcaOverTreatmentRevision/mlogitFitObj/fit.low10yrSurv.RData'
genFitSubData <- function(riskGrps, adjustedVars, depVar, refLevel, data, saveObjPathObjName){
  subData <- subset(data, riskStrata %in% riskGrps,select=c(depVar,adjustedVars))
  base::cat('nobs=', nrow(subData),'\n')
  rm(subData.mlogitFit)
  subData.mlogitFit <- gen.mlogitFit(droplevels(subData),adjustedVars,depVar, refLevel=refLevel)
cat('save obj to',saveObjPathObjName,'\n')
