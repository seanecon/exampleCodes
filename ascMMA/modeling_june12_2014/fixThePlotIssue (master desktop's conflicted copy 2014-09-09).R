...
names(anaDfDev)
levels(anaDfDev[,'fascsurgeongrpannual'])
list("[-Inf,14.6)")[[1]]
dummyEffectCoding.yz(anaDfDev,'fascsurgeongrpannual',levelList=list(levels(anaDfDev[,'fascsurgeongrpannual'])),typeVec='dummy',returnList=TRUE, codedVnList=list('fascDummy'))



dummycode.df.yz
anaDfDev=rename.vars(anaDfDev,c("fascsurgeongrpannual[14.6,22.1)","fascsurgeongrpannual[22.1, Inf]"),c("fascsurgeongrpMid","fascsurgeongrpHigh"))
...
fit.poly = glm(log(totopexp)~ 
              ...
              # +.qtr.since2004.sq #this will cause really back noMMAprediction for year after 2008
               ...
               +.qtr.since2008.pos
               #+.qtr.since2008.pos.sqrt not good
...
locList=valLoc.yz(anaDfDev$'.qtr.since2008')
...
totopexp.mma.noMma=list(data.frame(as.numeric(names(locList)),mean.totopexp.hat.mma, ci.totopexp.hat.mma[1,],ci.totopexp.hat.mma[2,])
...
plotLineCi.yz(totopexp.mma.noMma
              ...
              ,cex.legend.text=1
              ,sfrac=0.002
              ,pdf.path=pdf('Z:/j_scrdata/ascMMA/result/fig/totalOutPatientSpendingMmaNoMma_sep092014.pdf') #with e.g., C;/abc.pdf
)
