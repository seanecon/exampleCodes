...
fit.totop.perca=glm(log(percap.totopexp) ~ qtr + mean.age + .avg.comorbsum 
                 + mma2008.impact 
                 + fascsurgeongrpannual
                 #+ firstqtraftermma
                 #          +.qtr.since2006+.qtr.since2006.sqrt
...
summary(fit.totop.perca)

fit.totip.perca=glm(log(percap.totipexp) ~ qtr + mean.age + .avg.comorbsum 
        + mma2008.impact 
        + fascsurgeongrpannual
        #+ firstqtraftermma
                 +.qtr.since2006+.qtr.since2006.sqrt
...
#         +.qtr.since2006.bs9+.qtr.since2006.bs10+.qtr.since2006.bs11
        , data=anaDfwithBs.less)
summary(fit.totip.perca)



fit.perca.op=glm(log(percap.totopexp) ~ qtr + mean.age + .avg.comorbsum 
                 + mma2008.impact 
                 + fascsurgeongrpannual
                 #+ firstqtraftermma
                 +.qtr.since2006+.qtr.since2006.sqrt
                 ...
                 #         +.qtr.since2006.bs9+.qtr.since2006.bs10+.qtr.since2006.bs11
                 , data=anaDfwithBs.less)
summary(fit.perca.op)



fit.outpvol=glm(totaloutpvol ~  mma2008.impact+fascsurgeongrp+ mma2008.impact*fascsurgeongrpannual+mean.age+.avg.comorbsum+race.0+race.1+race.2+race.3+race.4+race.5+sex.1+.qtr.since2006.bs1+.qtr.since2006.bs2+.qtr.since2006.bs3+.qtr.since2006.bs4+.qtr.since2006.bs5+.qtr.since2006.bs6 +.qtr.since2006.bs7+.qtr.since2006.bs8+.qtr.since2006.bs9+.qtr.since2006.bs10+.qtr.since2006.bs11,
, offset=log(n.bene)
, family = poisson
, data=anaDfwithBs.less)
summary(fit.outpvol)
...
summary(fit.perca)
names(anaDfwithBs.less)
head(anaDfwithBs.less[,'percap.totexp'],10)

plot(ddply(anaDfwithBs.less,'.qtr.since2006',function(x){sum(x[,'n.bene'])}))
...
predVal=btXbWithShock.yz(coef(fit) #a named  usually coef(fit)[colnames(vcov(fit))]
                    , vcov(fit)  #at least colnamed matrix usually vcov(fit)
                    , changeEffect
                    , sd(fit$residuals)  # sd(fit$residuals) this is from the model  stardard error of shock
                    , model.matrix(formula(fit),fit$data) #usually it is  model.matrix(formula(fit),data)
                    , 100                   
...
meanVec.withUpcodingWithFascChange=apply(sumMat,1,mean)/1e9
ciMat.withUpcodingWithFascChange=apply(sumMat,1,function(x){quantile(x,probs=c(0.025,0.975))})/1e9

#MMA at all without upcoding but with change in FASC
changeEffect=c(mma2008.impact=0) #note mma2008.impact is actually whether have upcoding or not
locList=valLoc.yz(fit$data$'.qtr.since2006')
predVal=btXbWithShock.yz(coef(fit) #a named  usually coef(fit)[colnames(vcov(fit))]
                         , vcov(fit)  #at least colnamed matrix usually vcov(fit)
                         , changeEffect
...
meanVec.noUpcodingWithFascChange=apply(sumMat,1,mean)/1e9
ciMat.noUpcodingWithFascChange=apply(sumMat,1,function(x){quantile(x,probs=c(0.025,0.975))})/1e9


#no upcoding no FASC change at all (i.e., closest to no MMA at all case)
endof2007.fasc=subset(fit$data, .qtr.since2006==8)[,c('hrrnum','fascsurgeongrpannual')]
...
predVal=btXbWithShock.yz(coef(fit) #a named  usually coef(fit)[colnames(vcov(fit))]
                         ...
                         , changeEffect
                         , sd(fit$residuals)  # sd(fit$residuals) this is from the model  stardard error of shock
                         , model.matrix(formula(fit),dataChangedFascGrp) #usually it is  model.matrix(formula(fit),data)
                         , 100                   
...
meanVec.noUpcodingNoFascChange=apply(sumMat,1,mean)/1e9
ciMat.noUpcodingNoFascChange=apply(sumMat,1,function(x){quantile(x,probs=c(0.025,0.975))})/1e9


#upcoding no fasc change
changeEffect=NULL #note mma2008.impact is actually whether have upcoding or not
predVal=btXbWithShock.yz(coef(fit) #a named  usually coef(fit)[colnames(vcov(fit))]
                         , vcov(fit)  #at least colnamed matrix usually vcov(fit)
                         , changeEffect
                         , sd(fit$residuals)  # sd(fit$residuals) this is from the model  stardard error of shock
...
meanVec.upcodingNoFascChange=apply(sumMat,1,mean)/1e9
ciMat.upcodingNoFascChange=apply(sumMat,1,function(x){quantile(x,probs=c(0.025,0.975))})/1e9


plot(5*meanVec.withUpcodingWithFascChange,ylim=5*c(1.6,3.5),type='b', pch=1,lwd=1, main='MMA impact on unadjusted outpatient major surgery related expenditure', ylab='billion dollars')
lines(5*meanVec.noUpcodingNoFascChange,type='b',pch=2, lwd=1)
lines(5*meanVec.noUpcodingWithFascChange,type='b',pch=3, lwd=1)
lines(5*meanVec.upcodingNoFascChange,type='b',pch=4, lwd=1)
#lines(sumCostVec.true/1e9,type='b',col='green')
legend('bottomright',legend=c('MMA total effect: upcoding and FASC supply change','no MMA at all: no upcoding and no change in FASC supply','MMA partial effect through change in FASC supply (no upcoding effect)', "MMA partical effect through upcoding (no FASC change)"),pch=c(1,2,3,4),cex=0.75)
abline(v=8,lty=3)
