...
#   names(numFascDocBid.HrrQtrDf)
  ...
  anaDf.raw.4=subset(join(anaDf.raw.3, arfIndepClean, by='hrrnum'),year %in% 2004:2011)
  
  
  #the following is to add ASC 
(load('Z:/j_scrdata/ascMMA/collegeEduFromACS.20042011.RData'))
    
  #add variable ".qtr.since2006"

  newDf=data.frame(collegeEduFromACS.20042011[rep(seq(nrow(collegeEduFromACS.20042011)),each=4),],qtr=rep(seq(4),nrow(collegeEduFromACS.20042011)))
  newDf[,".qtr.since2006"] = (newDf[,'year']-2006)*4+newDf[,'qtr']
  
 anaDf.raw.5= join(anaDf.raw.4,newDf[,c('year','.qtr.since2006','collegeAbovePct','hrrnum')],by=c('hrrnum','.qtr.since2006'))
  
  #save(arfCleanedHrrYear20042011,file='Z:/j_scrdata/ascMMA/arfCleanedHrrYear20042011.RData')
  (load(file='Z:/j_scrdata/ascMMA/arfCleanedHrrYear20042011.RData'))
  
  newDf.1=data.frame(arfCleanedHrrYear20042011[rep(seq(nrow(arfCleanedHrrYear20042011)),each=4),],qtr=rep(seq(4),nrow(arfCleanedHrrYear20042011)))

  newDf.1[,".qtr.since2006"] = (newDf.1[,'year']-2006)*4+newDf.1[,'qtr']
 names(newDf.1)
  anaDf.raw.6= join(anaDf.raw.5,newDf.1[,c('year','.qtr.since2006', "per10kMd",           "percaincome" , "povpct",'hrrnum')],by=c('hrrnum','.qtr.since2006'))
  return(anaDf.raw.6)
}

anaDfNoBs=getAnaDfNoBs(3)
#save(anaDfNoBs, file='Z:/j_scrdata/ascMMA/result/anaDfNoBs.Jan062015.RData')
save(anaDfNoBs, file='Z:/j_scrdata/ascMMA/result/anaDfNoBs.Jan262015.RData')
...
save(anaDfWithBs, file='Z:/j_scrdata/ascMMA/result/anaDfWithBs.Jan262014.RData')

(load('Z:/j_scrdata/ascMMA/result/anaDfWithBs.Jan262014.RData'))
names(anaDfWithBs)
head(anaDfWithBs[,c('entday.opfascof','totaloutpvol')])
ddply(anaDfWithBs,'year',function(x){sum(x[,'totaloutpvol'])})
